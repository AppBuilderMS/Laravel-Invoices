            ...
            <div class="card mg-b-20">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-start">
                        ...
                        <a href="{{route('archivedPartiallyPaidInvoicesExportExcel')}}"  class="btn btn-success mb-2 ml-1 mr-1">
                        ...
                        <table id="example" class="table key-buttons text-md-nowrap table-striped table-hover table-bordered">
                            ...
                            @foreach($invoices as $index => $invoice)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{route('archivedInvoiceDetails-show', $invoice->id)}}" class="btn btn-ghost-info btn-sm" target="_blank">
                                            <i class="la la-eye"></i> {{trans('projectlang.preview')}}
                                        </a>

                                        <a href="{{route('archivedInvoices.edit', $invoice->id)}}" class="btn btn-ghost-info btn-sm">
                                            <i class="la la-edit"></i> {{trans('projectlang.edit')}}
                                        </a>

                                        <a href="{{route('printArchivedInvoice', $invoice->id)}}" class="btn btn-ghost-info btn-sm">
                                        ...
                                        <button class="btn btn-ghost-success btn-sm"
                                            ...
                                            <i class="la la-box-open"></i> {{trans('projectlang.restore_archived_invoice')}}
                                        </button>
                                        {{--@endif--}}
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">{{trans('projectlang.restore_archived_invoice_alert')}}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{route('restoreArchived', 'test')}}" method="POST">
                    @csrf
                    @method('PATCH')
                    <div class="form-group">
                        <p class="text-danger font-weight-bold">{{trans('projectlang.restore_archived_invoice_alert_text')}}</p>
                    ...
                    <div class="form-group">
                        ...
                        <button type="submit"  class="btn btn-success">{{trans('projectlang.restore_archived_invoice')}}</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form action="{{route('restoreArchived', 'test')}}" method="POST">
                    ...
                    <div class="form-group">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">{{trans('projectlang.close')}}</button>
                        <button type="submit" id="archiveBtn" class="btn btn-success">{{trans('projectlang.restore_archived_invoice')}}</button>
                    </div>
                </form>

                <form action="{{route('archivedInvoices.forceDelete', 'test')}}" method="POST">
                    @csrf
                    @method('DELETE')
                    <div class="form-group">
    ...
    <script>
        ...
        $(document).ready(function () {
            $('#deleteModal').on('show.bs.modal', function (event) {
                ...
                $('.forceDeleteBtn').css({'right':'148px', 'left': '182px'})
